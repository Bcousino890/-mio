# Brief P2 — Núcleo del localizador

> **Antes de empezar**: lee `docs/PLAN-LOCALIZADOR-IA-CL.md` completo, sobre todo §3 (el embudo), §4 (fases F1, F4b, F5b, F7c, F8) y §10 (orquestación). Este brief no reemplaza el plan, lo resume.

## Tu puesto

Eres el puesto **P2**. Construyes el corazón del sistema: el que decide qué ROL corresponde a cada propiedad. **Tus piezas son las de mayor riesgo del proyecto** — un peso mal calibrado une propiedades que no son la misma o confirma ROLes equivocados en fichas reales de clientes.

- **Modelo**: `/model claude-fable-5` con esfuerzo alto de razonamiento.
- **`/code-review` obligatorio antes de CADA push.** Sin excepciones.
- **Rama**: `feat/localizador-p2-nucleo`
- **Migración reservada**: `0100`
- **Arrancas YA** — no dependes de nadie para F1.
...
scraper/lib/locator-feeder-cl.mjs       (nuevo — pero su registro en el worker lo hace P1)
...
| `normalizeClRol()` | `web/lib/rol-format.ts` | Formato canónico de ROL (crítico: hay dos formatos conviviendo) |

## Tus tareas, en orden

### 1. F1 — Localizador determinista por clúster (lo primero, desbloquea a P4)

Esto cierra la "Fase 7/H11" pendiente: `property_cl.rol_matriz` y `rol_confidence` existen desde la migración `0064` y están **vacíos**.

**Migración `0100`**: tabla `property_locator_cl` + columnas `property_cl.rol_match_method`, `rol_matched_at`. Esquema exacto en §4-F1 del plan — es un **contrato congelado** (§10.3), P4 lo lee.
...
- **Dirección**: `exact_address` si existe; si no, la más larga con número entre los miembros → `address_full` (activa el camino de log-odds 4.2 de `addressEvidence`, la señal más fuerte).
...
**Endpoint** `POST /api/chile/property-locator {limit}`: clústeres pendientes priorizando `listing_count DESC, corredora_count DESC` (más evidencia primero). Auth con token interno — el patrón exacto está en `scraper/process-uploads.mjs:10-14` → `/api/admin/process-uploads-worker`.

**Feeder** `scraper/lib/locator-feeder-cl.mjs`: hace `fetch(APP_URL + '/api/chile/property-locator')`. Pídele a P1 que registre la cola `locator-feeder-cl` con cron `37 * * * *`.
...
### 2. F4b — Señal de footprint (cuando P1 tenga `parcel_footprint_stats_cl` en main)
...
**Crítico — el dataset es de mayo 2023**: si `anio_construccion` SII > 2022 o el anuncio dice "nueva/a estrenar", `building_count = 0` **NO penaliza** (la señal se anula, no resta). Sin esto, todas las casas nuevas quedarían mal puntuadas.

### 3. F5b — Re-score visual (cuando P3 y P4 tengan lo suyo)

Reusa `verifyCandidatesVisually()` de `web/lib/visual-match-cl.ts:71` (de P3) dentro de `locateProperty()`, **solo** cuando queda `needs_review` con top-2 cercanos: 4-8 mejores fotos exteriores vs tiles de los top-4 candidatos. Inyecta `visual_score` y re-puntúa — el camino de rescore ya existe en `captar-pipeline.ts` (~línea 895). Cachea los veredictos en `evidence`.

Abstrae el proveedor de tiles tras un env `SAT_TILE_PROVIDER`: los términos de uso comercial de Esri World Imagery son un hallazgo abierto (alternativas: Mapbox Static 50k/mes, Google Static 10k/mes).

### 4. F7c — Calibración continua

`scraper/calibrate-locator-cl.mjs`, cron semanal: regresión logística sobre `locator_labels_cl.signals` (el corpus que llenan P1 y P4) → propone pesos log-odds recalibrados para `scoreCandidateV3`, `footprintEvidence` y los pesos de par del dedup, con datos chilenos reales en vez de los pesos a ojo actuales. Es álgebra en CPU, corre en segundos, sin API.
...
Extiende `CL_PAIR_WEIGHTS`/`CL_HARD_SIGNALS` en `score-pair-cl.mjs` leyendo señales **ya calculadas** por P3 (coste marginal $0):

| Señal nueva | Peso propuesto | Origen |
|---|---|---|
| Mismo ROL localizado | **+0.70** (casi decisoria) | tu F1 |
| `numero_casa_ocr` igual | +0.45 | P3 F3 |
| Embeddings de foto near-dup | +0.45 | F6 |
| Mismo condominio/calle+nº (NLP) | +0.35 | P3 F2 |
| Pistas geocodificadas <100 m | +0.15 | P3 F2 |
| Atributos visuales compatibles | +0.10 | P3 F3 |
| **Atributos visuales incompatibles** | **−0.40** (guardarraíl) | P3 F3 |
...
- Migraciones idempotentes; tests con deps inyectadas.
...
- **Nunca** edites archivos de otro puesto — pide el cambio. En particular `worker-cl.mjs` es de P1.
