...
El header `Authorization` solo es necesario si se define `PROCESSING_SECRET` (ver abajo). Si no se define, el endpoint no exige autenticación.

### Variables de entorno requeridas:

En `docker-compose.yml` o `.env`:
```env
UPLOAD_DIR=/tmp/casafari-uploads
DATABASE_URL=postgresql://user:password@db:5432/casafari
PROCESSING_SECRET=your-secure-secret   # opcional — si se define, el worker exige Authorization: Bearer <secret>
```
...
- Soporta `PROCESSING_SECRET` opcional vía header `Authorization: Bearer <secret>`
