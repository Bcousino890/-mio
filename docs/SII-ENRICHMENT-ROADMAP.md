...
| **Nota sobre datos de cierre (CBR)** | No existe un CSV público del Conservador con ventas + precio (registro por jurisdicción, ~70 CBR; el SII solo publica agregados). Por eso el AVM v1 usa precios de **oferta**. ⚠️ **El scraper del SII de transferencias NO es viable**: la consulta "Consultar transferencia de bienes raíces" solo muestra las transferencias en las que el usuario autenticado es comprador/vendedor (no permite mirar el precio de una propiedad ajena) y es un sistema protegido. Vía única para cierres → **proveedor comercial** (databam/TocToc) importado por CSV |
| Fase 6: importador de transacciones — `POST /api/admin/transacciones-upload` ingesta un CSV de compraventas a `sii_transacciones_cl` (autodetecta `;`/`,`, upsert por lotes con unnest); botón "Importar CSV" en el estado vacío de la pestaña Ventas. Reemplaza al scraper inviable como la vía real para precios de cierre | ✅ Hecho (2026-07-11) |
| Fase 6: watchlists con backend — migración `0055_watchlists_cl`, API `/api/chile/watchlists` (CRUD + novedades = anuncios de venta activos en la zona vs baseline). En el visor: botón "Seguir" en la zona dibujada, panel de zonas seguidas con badge de novedades, cargar/marcar visto/borrar. Entrega por email/push queda para un job (requiere SMTP) | ✅ Hecho (2026-07-11) |
| Backlog restante | Alertas de watchlists por email/push (job + SMTP), capa de ventas CBR en el mapa (pins/h3), AVM v2 con comparables `sii_comparables_h3_cl` recalibrado contra cierres una vez cargado un dataset |
