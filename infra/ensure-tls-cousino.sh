#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# casafari-mio · ensure-tls-cousino.sh — emite/redeploya el certificado Let's
# Encrypt de $DOMAIN (sitio web-cousino) en el nginx COMPARTIDO, sin tocar
# casafari.conf (crm.cremme.es) ni nada ajeno. Mismo patrón aditivo que
# ensure-tls.sh, pero con su propio archivo de conf: casafari-cousino.conf.
#
# Llamado por bootstrap-cousino.sh y deploy-cousino.sh. Idempotente.
#
...
CONF_NAME="casafari-cousino.conf"
...
deploy_ssl() {
  ...
  docker cp "$REPO_DIR/infra/nginx-cousino-shared-ssl.conf" "$SHARED_NGINX:/etc/nginx/conf.d/$CONF_NAME"
  if docker exec "$SHARED_NGINX" nginx -t >/dev/null 2>&1; then
    ...
    docker cp "$REPO_DIR/infra/nginx-cousino-shared.conf" "$SHARED_NGINX:/etc/nginx/conf.d/$CONF_NAME"
...
if certbot certonly --manual --preferred-challenges http \
    ...
    --deploy-hook "env DOMAIN=$DOMAIN SHARED_NGINX=$SHARED_NGINX bash $REPO_DIR/infra/ensure-tls-cousino.sh"; then
  if ! crontab -l 2>/dev/null | grep -q "casafari-cousino"; then
    (crontab -l 2>/dev/null; echo "23 3 * * * env DOMAIN=$DOMAIN SHARED_NGINX=$SHARED_NGINX certbot renew --quiet # casafari-cousino") | crontab -
  fi
else
